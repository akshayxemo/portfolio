...
export default function Education({ id, title, from, year, marks, status }) {
  return (
    <>
      <div
        id={id}
        className="dark:text-white mb-4 last:mb-0 text-black p-6 dark:bg-slate-500/10 bg-slate-500/5 rounded-lg"
      >
        <div className="flex flex-row justify-between max-xsm:flex-wrap gap-3 mb-3">
          <div>
            ...
            {status === "Pursuing" ? (
              <span className="rounded-full py-1 px-4 bg-green-500/40 dark:text-green-400 text-green-700">
                {status}
              </span>
            ) : (
              <span className="rounded-full py-1 px-4 bg-sky-500/40 dark:text-sky-400 text-sky-700">
                {status}
              </span>
            )}
          </div>
